...
func closeChannel(final_proof string, wal string) (proof, wallet string) {

	result_txID := nmc_wallet.NmcVerifyProofTest(final_proof)
	fmt.Println(result_txID)

	parsed_txID := result_txID.Result[0]

	spew.Dump(parsed_txID)

	rawProof := nmc_wallet.NmcGetRawTransactionTest(wal, parsed_txID)
